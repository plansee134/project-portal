 ...
 * REVISED: Multi-Unit Support & Enhanced Order
 ******************************************************/
...
class AppState {
  ...
  set currentUnitIndex(index) { 
    this._currentUnitIndex = Math.max(0, Math.min(index, this.currentUnits.length - 1)); 
  }
...
class AppController {
  ...
  static async loadClientDashboard(authData) {
    ...
    // Store units for multi-unit navigation
    appState.currentUnits = authData.units || [];
    
    if (appState.currentUnits.length > 0) {
      this.setupUnitNavigation(appState.currentUnits);
      await this.loadUnitData(appState.currentUnits[0].sd06Code, 0);
    } else if (authData.sd06Code) {
      // Single unit case
      appState.currentUnits = [{ sd06Code: authData.sd06Code }];
      await this.loadUnitData(authData.sd06Code, 0);
    } else {
  ...
  static setupUnitNavigation(units) {
    ...
    if (units.length > 1) {
      ...
      container.innerHTML = units.map((unit, index) => {
        ...
        const subtitle = unit.unitName && unit.compound ? unit.unitName : '';
        const isActive = index === 0 ? 'active' : '';
        
        return `
          <button class="unit-tab ${isActive}" 
                  onclick="AppController.switchUnit('${unit.sd06Code}', ${index})"
  ...
  static async switchUnit(sd06Code, index) {
    const appState = AppState.getInstance();
    ...
    // Update dashboard title for multi-unit
    if (appState.currentUnits.length > 1) {
      const currentUnit = appState.currentUnits[index];
      UIHelper.setText('dashboardTitle', `${currentUnit.compound || currentUnit.unitName || 'Unit'} Details`);
      UIHelper.setText('dashboardSubtitle', currentUnit.compound ? `at ${currentUnit.compound}` : 'Project Details');
      UIHelper.setText('unitCount', `Unit ${index + 1} of ${appState.currentUnits.length}`);
    }
    
    // Load unit data
    await this.loadUnitData(sd06Code, index);
  }
  
  static async loadUnitData(sd06Code, index = 0) {
    try {
      ...
      this.renderClientData(report, index);
  ...
  static renderClientData(report, unitIndex = 0) {
    const { unit, design, execution, executionTimeline, view3D, currentPhase } = report;
    const appState = AppState.getInstance();
    
    // Update dashboard header for multi-unit
    if (appState.currentUnits.length > 1) {
      const currentUnit = appState.currentUnits[unitIndex];
      UIHelper.setText('dashboardTitle', `${currentUnit.compound || currentUnit.unitName || 'Unit'} Details`);
      UIHelper.setText('dashboardSubtitle', currentUnit.compound ? `at ${currentUnit.compound}` : 'Project Details');
      UIHelper.setText('unitCount', `Unit ${unitIndex + 1} of ${appState.currentUnits.length}`);
    }
    ...
    // Then Unit Information
    this.renderUnitInformation(unit, design);
  ...
  static renderUnitInformation(unit, design) {
    // Basic Information
...
function switchUnit(sd06Code, index) {
  AppController.switchUnit(sd06Code, index);
}
